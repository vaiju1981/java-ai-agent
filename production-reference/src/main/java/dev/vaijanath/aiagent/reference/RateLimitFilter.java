 ...
 * larger than a cap with 413 and throttles callers to a per-minute token bucket with 429. The bucket
 * is keyed by API key, then principal, then remote address.
 *
 * <p>The bucket store is in-memory, so limits are per service instance — a multi-replica deployment
 * needs a shared store (e.g. Redis) for a global limit. Adequate as a baseline and for single-node.
 */
class RateLimitFilter extends OncePerRequestFilter {
    ...
    private static String clientKey(HttpServletRequest request) {
        ...
        String principal = request.getHeader("X-Principal-Id");
